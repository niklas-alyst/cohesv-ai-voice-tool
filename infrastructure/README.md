...
# Step 2: Build and push data-api-authorizer image (REQUIRED before shared stack)
make push-data-api-authorizer ENV=dev

# Step 3: Deploy shared infrastructure (includes authorizer Lambda)
./infrastructure/deploy.sh dev shared

# Step 4: Build and push remaining service images
...
# Or after images are pushed, deploy all at once:
...
The Data API uses API key authentication via a Lambda authorizer. The API key is automatically generated when deploying the shared stack.

### Retrieving the API Key

After deploying the shared infrastructure, retrieve the auto-generated API key:
...
- **First deployment**: API key is auto-generated by CloudFormation
- **Subsequent deployments**: API key persists (does not change)
- **Key rotation**: Update the secret in Secrets Manager manually or delete/recreate the stack
...
- **Two API Gateways with different authentication**:
  - **Webhook API**: Twilio signature validation (no API key required)
