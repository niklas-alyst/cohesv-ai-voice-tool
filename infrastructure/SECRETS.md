...
Secrets (API keys, auth tokens) are stored in AWS Secrets Manager and injected into Lambda functions via CloudFormation dynamic references. The system uses two types of secrets:

1. **Manual secrets** - Created from `.env` files via Makefile (Twilio, OpenAI)
2. **Auto-generated secrets** - Created automatically by CloudFormation during deployment (Data API key)
...
WUNSE_API_BASE_URL=https://api.wunse.com
WUNSE_API_KEY=your-actual-wunse-api-key
```
...
This will create four secrets:
- `{ENV}/twilio/account-sid`
- `{ENV}/twilio/auth-token`
- `{ENV}/openai/api-key`
- `{ENV}/wunse/api-key`

**Note:** The Data API key (`{ENV}/ai-voice-tool/data-api-key`) is NOT created via Makefile - it's auto-generated by CloudFormation when deploying the shared stack (see "Auto-Generated Secrets" section below).
...
WunseApiKeySecretArn:
arn:aws:secretsmanager:ap-southeast-2:404293832854:secret:dev/wunse/api-key-def456
```
...
| `make secrets-create-wunse-api-key` | Create only the Wunse API key secret |
...
| `make secrets-update-wunse-api-key` | Update only the Wunse API key secret |
...
The Data API key is **automatically generated** by CloudFormation when you deploy the shared infrastructure stack. Unlike other secrets, you don't need to create a `.env` file or use Makefile commands.

**How it works:**
1. Deploy the shared stack: `./infrastructure/deploy.sh dev shared`
2. CloudFormation creates the secret `{ENV}/ai-voice-tool/data-api-key`
3. AWS generates a secure 48-character random string
4. The authorizer Lambda uses this key to validate API requests
...
**Key lifecycle:**
- **First deployment**: Key is auto-generated
- **Subsequent deployments**: Key persists (does not change)
- **Rotation**: Update the secret manually via AWS CLI or delete/recreate the stack

**Manual rotation example:**
```bash
# Generate new key
NEW_KEY=$(openssl rand -base64 36)
...
See [../data-api-authorizer/README.md](../data-api-authorizer/README.md) for more details on how the Data API key is used.

## Service Secret Requirements

Different services require different sets of secrets:

- **voice-parser** - Requires Twilio credentials, OpenAI API key, and Wunse API key for customer lookup
- **webhook-handler** - Requires Twilio auth token and Wunse API key for customer lookup
- **data-api-server** - Only reads from S3 (no API keys needed, authentication handled by authorizer)

Services with minimal secrets still have `.env.example` files for consistency.
